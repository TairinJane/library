...
class BooksController(
    ...
    fun books(
        ...
        @RequestParam(defaultValue = "false") available: Boolean
    ): List<BookAvailableDTO> {
    ...
    fun lendBook(
        ...
        try {
            ...
            val borrowedBook = borrowedBooksRepository.save(
                BorrowedBook(
                    book = book, reader = reader, employee = employee, dueDate = dueDate
                )
            )
            println("success: $bookId $readerId $dueDate")
            return borrowedBook
        } catch (e: Exception) {
